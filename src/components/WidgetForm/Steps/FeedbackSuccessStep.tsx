...
export function FeedbackSuccessStep({ onFeedbackRestartRequest, language, theme }: FeedbackSuccessStepProps) {
  ...
  return (
    <>
      <header className={`flex items-center justify-end w-full ${theme === 'light' ? 'pb-3 mb-2 border-b border-gray-200' : 'pb-2'}`}>
      ...
      <div className="flex flex-col items-center py-8 md:py-10 w-full max-w-[304px] px-2">
        ...
        <span className={`text-lg md:text-xl mt-2 text-center font-medium ${themeClasses.textPrimary}`}>{t.success.message}</span>
